  ...
  targets.map(target => {
    switch (target) {
      case 'windows':
        ...
        break;
      case 'linux':
        gdnlibArr.splice(entryInsertPoint, 0, `X11.64="res://rust-modules/${name}/lib${name}.so"`);
        gdnlibArr.splice(depInsertPoint, 0, `X11.64=[  ]`);
        break;
      case 'osx':
        gdnlibArr.splice(entryInsertPoint, 0, `OSX.64="res://rust-modules/${name}/lib${name}.dylib"`);
